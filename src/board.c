...
void moveShip(Board *board, Ship *ship, Direction direction) {
  ...
  if (ship->orientation == HORIZONTAL) {
    ...
    case LEFT:
      ...
      removeTileShip(&board->tiles[(ship->head.x + ship->type - 1) +
                                   ship->head.y * board->WIDTH]);
      setTileShip(&board->tiles[position.x + position.y * board->WIDTH], ship);
      break;
    case RIGHT:
      if (getTileState(board, createCoordinate((ship->head.x + ship->type),
                                               ship->head.y)) == MINE) {
        ship->hits |= 1 << (ship->type - 1);
      }

      removeTileShip(&board->tiles[ship->head.x + ship->head.y * board->WIDTH]);
      setTileShip(&board->tiles[(ship->head.x + ship->type) +
    ...
    case UP:
      ...
      removeTileShip(
          &board->tiles[ship->head.x +
                        (ship->head.y + ship->type - 1) * board->WIDTH]);
      setTileShip(&board->tiles[position.x + position.y * board->WIDTH], ship);
      break;
    case DOWN:
      if (getTileState(board, createCoordinate(ship->head.x,
                                               ship->head.y + ship->type)) ==
          MINE) {
        ship->hits |= 1 << (ship->type - 1);
      }

      removeTileShip(&board->tiles[ship->head.x + ship->head.y * board->WIDTH]);
      setTileShip(&board->tiles[position.x +
                                (position.y + ship->type - 1) * board->WIDTH],
                  ship);
      break;
    case LEFT:
    case RIGHT:
      for (i = 0; i < ship->type; ++i) {
        ...
        removeTileShip(
            &board->tiles[ship->head.x + (ship->head.y + i) * board->WIDTH]);
        setTileShip(&board->tiles[current.x + current.y * board->WIDTH], ship);
      }
      break;
    }
